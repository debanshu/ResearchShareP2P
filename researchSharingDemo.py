...
class FileShareWindow(gtk.Window):
	...
	labelGrp = gtk.Label("Publisher Group:")
        ...
        button.connect("clicked", self.publishDirectory, self.entryDir.get_text, entryKeywordGrp2,True)
        button.show()
        vbox.pack_start(hbox, expand=False, fill=False)
...
if __name__ == '__main__':
    ...
    try:
     ...
     fin = open("key.bin","wb")
     nid = entangled.node.EntangledNode._generateID(entangled.node.EntangledNode())
     fin.write(nid)
     fin.close()		
    #node = entangled.node.EntangledNode(id = nid ,udpPort=int(sys.argv[1]), dataStore=dataStore)
    node = entangled.node.EntangledNode(udpPort=int(sys.argv[1]), dataStore=dataStore)
    #test = entangled.node.EntangledNode._generateID(entangled.node.EntangledNode()) 
    #print test
    print node.id
    #node.invalidKeywords.extend(('mp3', 'png', 'jpg', 'txt', 'ogg'))
    node.invalidKeywords.extend(())
